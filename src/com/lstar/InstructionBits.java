...
public class InstructionBits {
    ...
    private void handleCall(String ist, String op1, String op2, int nu) throws Exception{
        if(nu == 2){
            ...
            bits = bits + "00";
            if(!PicoBlazeCompilator.call.containsKey(op1.trim())){
                throw new Erro(4);
            }
            bits = bits + integerTostring(PicoBlazeCompilator.call.get(op1.trim()), 10);
        }else {
            if(argumentumNumberError(3,nu)) {
                ...
                if(!PicoBlazeCompilator.call.containsKey(op2.trim())){
                    throw new Erro(4);
                }
                bits = bits + integerTostring(PicoBlazeCompilator.call.get(op2.trim()), 10);
            }
        }
    }
    ...
    private boolean isRegister(String op) throws Exception{
        if(op != null && PicoBlazeCompilator.namereg.containsKey(op)){
            return true;
        }
        if(op != null && PicoBlazeCompilator.namereg.containsValue(op.toUpperCase())) throw new Erro(6);
        try {
    ...
    private boolean isOperand2(String op){
        if(op != null && PicoBlazeCompilator.constant.containsKey(op)){
            return true;
        }
        return false;
    }
    ...
    private int getOperand2(String op) throws Exception {
        if(PicoBlazeCompilator.constant.containsKey(op)){
            return Integer.parseInt(PicoBlazeCompilator.constant.get(op),16);
        }
    ...
    private int getRegisterNumber(String op) throws Exception {
        if(isRegister(op) && PicoBlazeCompilator.namereg.containsKey(op)){
            return Integer.parseInt(PicoBlazeCompilator.namereg.get(op).charAt(1) + "",16);
        }
